...
public final class PixelGrid {
    ...
    public int get(int x, int y) { return data[y * width + x]; }

    /**
     * Returns the backing array. Read-only contract: do not mutate.
     * Use {@link #mutableCopy()} when you need to modify pixels.
     */
    public int[] data() { return data; }

    /** Create a mutable copy of the pixel data (for editor strokes, undo snapshots). */
    public int[] mutableCopy() { return Arrays.copyOf(data, data.length); }
    ...
    /** True if all pixels have zero alpha. */
