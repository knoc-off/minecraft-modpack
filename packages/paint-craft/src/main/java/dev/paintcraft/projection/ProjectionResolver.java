...
public final class ProjectionResolver {
    ...
    public static ProjectionResult resolve(Decal decal, BlockGetter level) {
        ...
        return new ProjectionResult(
            new ResolvedSurface(fragments),
            new ProjectionState(depthBuf, candidates)
        );
    }
     ...
     */
    public static ProjectionResult resolveIncremental(
            Decal decal, BlockGetter level,
            ProjectionState previous,
            List<SurfaceFragment> oldFragments,
            Set<BlockPos> changedBlocks) {

        if (previous == null || !previous.canResolveIncrementally()) {
        ...
        // Reuse old fragments outside dirty region, rebuild only dirty ones
        List<SurfaceFragment> fragments = new ArrayList<>();

        if (oldFragments != null) {
            for (SurfaceFragment frag : oldFragments) {
                if (frag.u1() < dirtyPxMinX || frag.u0() > dirtyPxMaxX
                    || frag.v1() < dirtyPyMinY || frag.v0() > dirtyPyMaxY) {
                    fragments.add(frag);
                }
            }
        }

        // Rebuild only candidates overlapping the dirty region
        for (FaceCandidate c : allCandidates) {
            int px0 = vol.toPixelX(c.u0, wPx);
            int py0 = vol.toPixelY(c.v0, hPx);
            int px1 = vol.toPixelXMax(c.u1, wPx);
            ...
            if (px1 < dirtyPxMinX || px0 > dirtyPxMaxX
                || py1 < dirtyPyMinY || py0 > dirtyPyMaxY) {
                continue;
            }
        ...
        return new ProjectionResult(
            new ResolvedSurface(fragments),
            new ProjectionState(depthBuf, allCandidates)
        );
    }
    ...
    public static void collectFaces(Projection vol, AABB worldBox,
        ...
        for (Direction face : Direction.values()) {
            ...
            int dot = face.getStepX() * projNormal.getStepX()
                    + face.getStepY() * projNormal.getStepY()
                    + face.getStepZ() * projNormal.getStepZ();
            if (dot <= 0) continue;
