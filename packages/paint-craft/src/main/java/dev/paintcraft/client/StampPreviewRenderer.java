...
public final class StampPreviewRenderer {
    ...
    private static final UUID PREVIEW_UUID = new UUID(0L, 0L);
    ...
    public static void render(PoseStack poseStack, MultiBufferSource bufferSource, Vec3 cameraPos) {
        ...
        RenderType renderType = DecalRenderType.ghostDecal(previewTexture.location());
    ...
    private static void rebuildPreview(ItemStack stack, BlockPos pos, Direction face, Direction up) {
        ...
        ProjectionResult result = ProjectionResolver.resolve(previewDecal, Minecraft.getInstance().level);
        previewResolved = result != null ? result.surface() : null;

        // Always recreate texture to avoid dimension mismatches
        if (previewTexture != null) {
            previewTexture.close();
        }
        previewTexture = new DecalTexture(previewDecal.widthPx(), previewDecal.heightPx(), previewDecal.pixels());
    }
    ...
    private static void renderGhostFragment(VertexConsumer consumer, Matrix4f matrix,
        ...
        int ghostAlpha = dev.paintcraft.ModConfig.CONFIG.ghostPreviewOpacity.get();
